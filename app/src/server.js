...
const hostUsersString = process.env.HOST_USERS ||
...
if (
    turnServerEnabled && turnServerUrl && turnServerUsername &&
...
const ipWhitelist = {
    ...
    allowed: process.env.IP_WHITELIST_ALLOWED
        ? JSON.parse(process.env.IP_WHITELIST_ALLOWED)
        : [],
};
...
const OIDC = {
    ...
    config: {
        ...
        authRequired: process.env.OIDC_AUTH_REQUIRED
            ...
            : false, // Set to true if authentication is required for all routes
        auth0Logout: process.env.OIDC_AUTH_LOGOUT
            ? getEnvBoolean(process.env.OIDC_AUTH_LOGOUT)
            : true, // Set to true to enable logout with Auth0
        routes: {
...
app.get("/join/", async (req, res) => {
    if (Object.keys(req.query).length > 0) {
        ...
        */
        const { room, name, audio, video, screen, notify, hide, token } =
            checkXSS(req.query);
        ...
        if (!allowRoomAccess && !token) {
            return res.status(401).json({
                message: "Direct Room Join Unauthorized",
            });
        }
        ...
        const OIDCUserAuthenticated = OIDC.enabled &&
            req.oidc.isAuthenticated();

        // Peer valid going to auth as host
        if (
            (hostCfg.protected && isPeerValid && isPeerPresenter &&
                !hostCfg.authenticated) || OIDCUserAuthenticated
        ) {
            const ip = getIP(req);
            hostCfg.authenticated = true;
            authHost.setAuthorizedIP(ip, true);
...
app.post("/login", (req, res) => {
    ...
    if (isPeerValid) {
        ...
        const isPresenter = roomPresenters &&
            roomPresenters.includes(username).toString();
        const token = encodeToken({
            username: username,
            password: password,
...
app.get("/buttons", (req, res) => {
    res.status(200).json({
        message: config && config.buttons ? config.buttons : false,
    });
});
...
app.get("/brand", (req, res) => {
    res.status(200).json({
        message: config && config.brand ? config.brand : false,
    });
});
...
app.post("/slack", (req, res) => {
    ...
    const mySignature = "v0=" +
        CryptoJS.HmacSHA256(sigBaseString, slackSigningSecret);
...
function getServerConfig(tunnel = false) {
    return {
        ...
        host_protected: hostCfg.protected || hostCfg.user_auth
            ? hostCfg
            : false,
...
io.sockets.on("connect", async (socket) => {
        ...
        switch (method) {
            case "checkPeerName":
                ...
                for (let id in peers[room_id]) {
                    if (
                        peer_id != id &&
                        peers[room_id][id]["peer_name"] == peer_name
                    ) {
            ...
            case "getChatGPT":
                ...
                if (!configChatGPT.enabled) {
                    return cb({
                        message: "ChatGPT seems disabled, try later!",
                    });
                }
        ...
        if (hostCfg.user_auth || peer_token) {
            ...
            if (peer_token) {
                try {
                    ...
                    log.debug(
                        "[" + socket.id + "] JOIN ROOM - USER AUTH check peer",
                        {
                            ip: peer_ip,
    ...
    socket.on("roomAction", async (cfg) => {
        ...
        const { room_id, peer_id, peer_name, peer_uuid, password, action } =
            config;
            ...
            switch (action) {
                ...
                case "checkPassword":
                    const data = {
                        peer_name: peer_name,
                        action: action,
                        password: password == peers[room_id]["password"]
                            ? "OK"
                            : "KO",
                    };
    ...
    socket.on("peerName", async (cfg) => {
        ...
        for (let peer_id in peers[room_id]) {
            if (
                peers[room_id][peer_id]["peer_name"] == peer_name_old &&
                peer_id == socket.id
            ) {
                peers[room_id][peer_id]["peer_name"] = peer_name_new;
                // presenter
                if (
                    presenters && presenters[room_id] &&
                    presenters[room_id][peer_id]
                ) {
    ...
    socket.on("peerNickname", async (cfg) => {
        // Prevent XSS injection
        const config = checkXSS(cfg);
        // log.debug('Peer nickname', config);
        const { room_id, peer_id, peer_nickname } = config;
        const data = {
            peer_id: peer_id,
            peer_nickname: peer_nickname,
        };
        log.debug(
            "[" + socket.id + "] emit peerNickname to [room_id: " + room_id +
                "]",
            data,
        );
        await sendToRoom(room_id, socket.id, "peerNickname", data);
    });
        ...
        try {
            for (let peer_id in peers[room_id]) {
                if (
                    ...
                    switch (element) {
                        case "video":
                            peers[room_id][peer_id]["peer_video_status"] =
                                status;
                            break;
                        case "audio":
                            peers[room_id][peer_id]["peer_audio_status"] =
                                status;
                            break;
                        case "screen":
                            peers[room_id][peer_id]["peer_screen_status"] =
                                status;
                            break;
                        case "hand":
                            peers[room_id][peer_id]["peer_hand_status"] =
                                status;
                            break;
                        case "rec":
                            ...
                            break;
                        case "privacy":
                            peers[room_id][peer_id]["peer_privacy_status"] =
                                status;
                            break;
                        default:
                            break;
    ...
    socket.on("peerAction", async (cfg) => {
        ...
        if (send_to_all) {
            ...
            log.debug(
                "[" + socket.id + "] emit peerAction to [" + peer_id +
                    "] from room_id [" + room_id + "]",
            );
    ...
    socket.on("kickOut", async (cfg) => {
        ...
        if (isPresenter) {
            log.debug(
                "[" + socket.id + "] kick out peer [" + peer_id +
                    "] from room_id [" + room_id + "]",
            );
    ...
    socket.on("fileAbort", async (cfg) => {
        ...
        log.debug(
            "[" + socket.id + "] Peer [" + peer_name +
                "] send fileAbort to room_id [" + room_id + "]",
        );
    ...
    socket.on("fileReceiveAbort", async (cfg) => {
        ...
        log.debug(
            "[" + socket.id + "] Peer [" + peer_name +
                "] send fileReceiveAbort to room_id [" + room_id + "]",
        );
    ...
    socket.on("videoPlayer", async (cfg) => {
        ...
        if (peer_id) {
            log.debug(
                "[" + socket.id + "] emit videoPlayer to [" + peer_id +
                    "] from room_id [" + room_id + "]",
                data,
            );
    ...
    async function removePeerFrom(channel) {
        ...
        log.info(
            "[removePeerFrom] - active rooms and peers count",
            activeRooms,
        );
...
function isPeerPresenter(room_id, peer_id, peer_name, peer_uuid) {
    try {
        if (!presenters[room_id] || !presenters[room_id][peer_id]) {
            ...
            for (
                const [existingPeerID, presenter] of Object.entries(
                    presenters[room_id] || {},
                )
            ) {
                if (presenter.peer_name === peer_name) {
                    log.debug(
                        "[" + peer_id + "] Presenter found",
                        presenters[room_id][existingPeerID],
                    );
                    return true;
                }
            }
...
function isAuthPeer(username, password) {
    return hostCfg.users &&
        hostCfg.users.some((user) =>
...
function encodeToken(token) {
    ...
    const encryptedPayload = CryptoJS.AES.encrypt(payloadString, jwtCfg.JWT_KEY)
        .toString();
...
function isAllowedRoomAccess(logMessage, req, hostCfg, peers, roomId) {
    ...
    const allowRoomAccess = (!hostCfg.protected && !OIDC.enabled) || // No host protection and OIDC mode enabled (default)
