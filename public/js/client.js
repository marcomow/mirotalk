...
function setButtonsToolTip() {
    ...
    setTippy(
        msgerShowChatOnMsgDiv,
        "Show chat when you receive a new message",
        "bottom",
    );
    ...
    setTippy(
        networkIP,
        "IP address associated with the ICE candidate",
        "right",
    );
    setTippy(
        networkHost,
...
function refreshMainButtonsToolTipPlacement() {
    ...
    // ButtonsBar
    placement =
        btnsBarSelect.options[btnsBarSelect.selectedIndex].value == "vertical"
            ? "right"
            : "top";
    ...
    setTippy(
        audioBtn,
        useAudio ? "Stop the audio" : "My audio is disabled",
        bottomButtonsPlacement,
    );
    setTippy(
        videoBtn,
        useVideo ? "Stop the video" : "My video is disabled",
        bottomButtonsPlacement,
    );
 ...
 */
function getUUID() {
    const uuid4 = ([1e7] + -1e3 + -4e3 + -8e3 + -1e11).replace(
        /[018]/g,
        (c) =>
            (c ^
...
function getPeerToken() {
    if (window.sessionStorage.peer_token) {
        return window.sessionStorage.peer_token;
    }
    let qs = new URLSearchParams(window.location.search);
 ...
 * @returns {string} Peer Name
 */
function getPeerNickname() {
    const qs = new URLSearchParams(window.location.search);
    const name = filterXSS(qs.get("nickname"));
    if (isHtml(name)) {
        console.log("Direct join", { name: "Invalid name" });
        return "Invalid name";
    }
...
function initClientPeer() {
    ...
    signalingSocket.on("peerNickname", handlePeerName);
...
async function sendToDataChannel(config) {
    ...
    if (
        thereArePeerConnections() && typeof config === "object" &&
        config !== null
    ) {
...
function roomIsBusy() {
    ...
    Swal.fire({
        ...
        title: "Room is busy",
        html:
            `The room is limited to ${userLimits.count} users. <br/> Please try again later`,
...
function handleRules(isPresenter) {
    console.log(
        "14. Peer isPresenter: " + isPresenter +
            " Reconnected to signaling server: " + isPeerReconnected,
    );
    if (!isPresenter) {
...
function handleButtonsRule() {
    ...
    elemDisplay(
        captionBtn,
        buttons.main.showCaptionRoomBtn && speechRecognition,
    ); // auto-detected
    ...
    elemDisplay(
        snapshotRoomBtn,
        buttons.main.showSnapshotRoomBtn && !isMobileDevice,
    );
    elemDisplay(fileShareBtn, buttons.main.showFileShareBtn);
    elemDisplay(
        documentPiPBtn,
        showDocumentPipBtn && buttons.main.showDocumentPipBtn,
    );
    ...
    elemDisplay(
        msgerTogglePin,
        !isMobileDevice && buttons.chat.showTogglePinBtn,
    );
    ...
    elemDisplay(
        captionTogglePin,
        !isMobileDevice && buttons.caption.showTogglePinBtn,
    );
    elemDisplay(captionMaxBtn, !isMobileDevice && buttons.caption.showMaxBtn);
    // Settings
    elemDisplay(
        dropDownMicOptions,
        buttons.settings.showMicOptionsBtn && isPresenter,
    ); // auto-detected
...
function userNameAlreadyInRoom() {
    ...
    Swal.fire({
        ...
        title: "Username",
        html:
            `The Username is already in use. <br/> Please try with another one`,
        showDenyButton: false,
        confirmButtonText: `OK`,
        showClass: { popup: "animate__animated animate__fadeInDown" },
        hideClass: { popup: "animate__animated animate__fadeOutUp" },
...
async function loadLocalStorage() {
    ...
    if (localStorageDevices) {
        ...
        if (!initMicrophoneExist || !audioInputExist) {
            console.log(
                "12.1 Audio devices seems changed, use default index 0",
            );
        ...
        if (!initSpeakerExist || !audioOutputExist) {
            console.log(
                "12.2 Speaker devices seems changed, use default index 0",
            );
        ...
        if (!initVideoExist || !videoExist) {
            console.log(
                "12.3 Video devices seems changed, use default index 0",
            );
    ...
    function updateInitLocalVideoMediaStream(camStream) {
        if (camStream) {
            ...
            console.log(
                ...
                localVideoMediaStream.getVideoTracks()[0].getSettings(),
            );
        }
    }
...
function checkPeerAudioVideo() {
    ...
    if (audio) {
        ...
        let queryPeerAudio = useAudio
            ? audio === "1" || audio === "true"
            : false;
        if (queryPeerAudio != null) {
            handleAudio(audioBtn, false, queryPeerAudio);
        }
    ...
    if (video) {
        ...
        let queryPeerVideo = useVideo
            ? video === "1" || video === "true"
            : false;
        if (queryPeerVideo != null) {
            handleVideo(videoBtn, false, queryPeerVideo);
        }
...
async function handleAddPeer(config) {
    ...
    const peer_nickname = peers[peer_id]["peer_nickname"];
...
async function handleOnIceCandidate(peer_id) {
    peerConnections[peer_id].onicecandidate = (event) => {
        ...
        switch (type) {
            ...
            default:
                console.warn(
                    `[ICE candidate] unknown type: ${type}`,
                    candidate,
                );
                break;
        }
...
async function handleAddTracks(peer_id) {
    ...
    const videoTrack = localVideoMediaStream &&
        localVideoMediaStream.getVideoTracks()[0];
    const audioTrack = localAudioMediaStream &&
    ...
    if (videoTrack) {
        ...
        await peerConnections[peer_id].addTrack(
            videoTrack,
            ...
            .then((local_description) => {
                ...
                peerConnections[peer_id]
                    ...
                    .catch((err) => {
                        ...
                        userLog(
                            "error",
                            "Offer setLocalDescription failed " + err,
                        );
                    });
            })
            .catch((err) => {
                ...
                peerConnections[peer_id]
                    ...
                    .then((local_description) => {
                        ...
                        peerConnections[peer_id]
                            ...
                            .catch((err) => {
                                console.error(
                                    "[Error] answer setLocalDescription",
                                    err,
                                );
                                userLog(
                                    "error",
...
async function loadRemoteMediaStream(stream, peers, peer_id, kind) {
    ...
    switch (kind) {
        case "video":
            ...
            const peerVideoText = document.createTextNode(peer_nickname);
            ...
            remoteVideoAudioUrlBtn.setAttribute(
                "id",
                peer_id + "_videoAudioUrl",
            );
            ...
            remoteVideoFullScreenBtn.setAttribute(
                "id",
                peer_id + "_fullScreen",
            );
            ...
            if (!isMobileDevice) {
                ...
                setTippy(
                    remoteHandStatusIcon,
                    "Participant hand is raised",
                    "bottom",
                );
                setTippy(
                    remoteVideoStatusIcon,
                    "Participant video is on",
                    "bottom",
                );
                setTippy(
                    remoteAudioStatusIcon,
            ...
            buttons.remote.showPrivateMessageBtn &&
                remoteExpandContainerDiv.appendChild(remotePrivateMsgBtn);
            buttons.remote.showFileShareBtn &&
                remoteExpandContainerDiv.appendChild(remoteFileShareBtn);
            buttons.remote.showShareVideoAudioBtn &&
                remoteExpandContainerDiv.appendChild(remoteVideoAudioUrlBtn);
            buttons.remote.showKickOutBtn &&
                remoteExpandContainerDiv.appendChild(remotePeerKickOut);
            ...
            if (
                !isMobileDevice && peer_audio && buttons.remote.showAudioVolume
            ) {
            ...
            remoteMedia.style.name = peer_id +
                (peer_screen_status ? "_typeScreen" : "_typeCam");
            ...
            remoteVideoWrap.style.display = isHideALLVideosActive
                ? "none"
                : "block";
            ...
            setPeerVideoStatus(
                peer_id,
                peer_screen_status ? peer_screen_status : peer_video_status,
            );
        ...
        case "audio":
            ...
            // Change audio output...
            if (sinkId && audioOutputSelect.value) {
    ...
    if (btnPn && videoPlayer && cam) {
        btnPn.addEventListener("click", () => {
            ...
            if (isVideoPinned) {
                ...
                if (
                    isScreen || videoPlayer.style.name == peerId + "_typeScreen"
                ) {
                    videoPlayer.style.objectFit = "contain";
                }
        ...
        remoteVideoFocusBtn.addEventListener("click", (e) => {
            ...
            for (let child of children) {
                if (child.id != remoteVideoWrap.id) {
                    child.style.display = isHideALLVideosActive
                        ? "none"
                        : "block";
                }
            }
    ...
    if (!isMobileDevice) {
        ...
        if (ZOOM_CENTER_MODE) {
            ...
            video.addEventListener("wheel", (e) => {
                ...
                const scaleFactor = zoomDirection === "zoom-out"
                    ? ZOOM_OUT_FACTOR
                    : ZOOM_IN_FACTOR;
    ...
    if (buttons.local.showZoomInOutBtn) {
        zoomIn.addEventListener("click", () => {
            ...
            if (isVideoPrivacyMode(video)) {
                return userLog(
                    "toast",
                    "Zoom in not allowed if video on privacy mode",
                );
            }
            zoom = zoom + 0.1;
            setTransform();
...
function handlePictureInPicture(btnId, videoId, peerId) {
    ...
    btnPiP.addEventListener("click", () => {
        if (video.pictureInPictureElement) {
            ...
            if (
                (myVideoStatus &&
                    myVideoStatus.className === className.videoOff) ||
                (remoteVideoStatus &&
        ...
        video.addEventListener("leavepictureinpicture", (event) => {
            ...
            if (video.paused) {
                ...
                video.play().catch((error) => {
                    console.error(
                        "Error playing video after exit PIP mode:",
                        error,
                    );
                });
            }
...
function toggleExtraButtons() {
    const isButtonsBarHidden = buttonsBar.style.display === "none" ||
...
function setSwapCameraBtn() {
    navigator.mediaDevices.enumerateDevices().then((devices) => {
        const videoInput = devices.filter((device) =>
...
function setChatRoomBtn() {
    ...
    msgerShowChatOnMsg.addEventListener("change", (e) => {
        ...
        showChatOnMessage
            ...
            : msgPopup(
                "info",
                "Chat not will be shown, when you receive a new message",
                "top-end",
                3000,
            );
...
function setMyWhiteboardBtn() {
    ...
    document.addEventListener("click", (event) => {
        if (
            !whiteboardDropDownMenuBtn.contains(event.target) &&
            !whiteboardDropDownMenuBtn.contains(event.target)
        ) {
            elemDisplay(whiteboardDropdownMenu, false);
        }
...
async function snapshotRoom() {
    ...
    try {
        ...
        video.onplay = async () => {
            ...
            link.download = "Room_" + roomId + "_" + getDataTimeString() +
                "_snapshot.png";
            link.click();
    ...
    try {
        ...
        function cloneVideoElements() {
            ...
            [...getSlALL("video")].forEach((video) => {
                ...
                if (!video.srcObject || video.id === "videoAudioUrlElement") {
                    return;
                }
                ...
                const isPIPAllowed = !videoPlayer.classList.contains(
                    "videoCircle",
                ); // not in privacy mode
                ...
                isLocalVideo
                    ? console.log(
                        "DOCUMENT PIP LOCAL: PiP allowed? -----> " +
                            isPIPAllowed,
                    )
                    : console.log(
                        "DOCUMENT PIP REMOTE: PiP allowed? -----> " +
                            isPIPAllowed,
                    );
                ...
                const videoStatus = getId(
                    isLocalVideo
                        ? "myVideoStatusIcon"
                        : peer_id + "_videoStatus",
                );

                // Start observing for new videosStatus and class changes (video ON/OFF)
                if (videoStatus) {
                    observeElementClassChanges(videoStatus, "VideoStatus");
                }
            });
...
function setMySettingsBtn() {
    ...
    switchAudioPitchBar.addEventListener("change", (e) => {
        ...
        userLog(
            "toast",
            `${icons.pitchBar} Audio pitch bar ` +
                (isAudioPitchBar ? "ON" : "OFF"),
        );
        playSound("switch");
    });
    ...
    switchH264Recording.addEventListener("change", (e) => {
        ...
        userLog(
            "toast",
            `${icons.codecs} Recording prioritize h.264 ` +
                (recPrioritizeH264 ? "ON" : "OFF"),
        );
        playSound("switch");
    });
    ...
    themeCustom.check.onchange = (e) => {
        ...
        userLog(
            "toast",
            `${icons.theme} Custom theme keep ` +
                (themeCustom.keep ? "ON" : "OFF"),
        );
        playSound("switch");
        e.target.blur();
    };
            ...
            switch (key) {
                ...
                case "d":
                    if (!showDocumentPipBtn) {
                        toastMessage(
                            "warning",
                            "The document PIP is not supported in this browser",
                        );
                        break;
                    }
...
async function getAudioVideoConstraints() {
    ...
    if (videoConstraints) {
        videoConstraints = await getVideoConstraints(
            videoQualitySelect.value ? videoQualitySelect.value : "default",
        );
...
function shareRoomByEmail() {
    Swal.fire({
        ...
        preConfirm: () => {
            ...
            const roomPassword = isRoomLocked && thisRoomPassword
                ? "Password: " + thisRoomPassword + newLine
                : "";
            const email = "";
            const emailSubject =
                `Please join our MiroTalk P2P Video Chat Meeting`;
            const emailBody =
                `The meeting is scheduled at: ${newLine} DateTime: ${selectedDateTime} ${newLine}${roomPassword}Click to join: ${roomURL} ${newLine}`;
            document.location = "mailto:" + email + "?subject=" + emailSubject +
                "&body=" + emailBody;
        },
    });
...
function handleAudio(e, init, force = null) {
    ...
    force != null
        ? (e.className = audioClassName)
        : (e.target.className = audioClassName);
...
async function toggleScreenSharing(init = false) {
    try {
        ...
        if (!isScreenStreaming) {
            ...
            console.log(
                "My video status before screen sharing: " + myVideoStatusBefore,
            );
        } else {
            if (!useVideo && !useAudio) {
                return handleToggleScreenException(
        ...
        const screenMediaPromise = isScreenStreaming
            ? await navigator.mediaDevices.getUserMedia(
                await getAudioVideoConstraints(),
            )
        ...
        if (screenMediaPromise) {
            ...
            if (init) {
                ...
                if (hasVideoTrack(initStream)) {
                    const newInitStream = new MediaStream([
            ...
            if (myPrivacyBtn) {
                isScreenStreaming
                    ? elemDisplay(myPrivacyBtn, false)
...
async function refreshMyStreamToPeers(stream, localAudioTrackChange = false) {
    ...
    if (useAudio && localAudioTrackChange) {
    ...
    const audioStream = streamHasAudioTrack
        ? stream
    ...
    const videoStream = streamHasVideoTrack
        ? stream
        : localStreamHasVideoTrack && localVideoMediaStream;
    ...
    for (const peer_id in peerConnections) {
        ...
        // Replace video track
        const videoSender = peerConnections[peer_id].getSenders().find((s) =>
        ...
        // Replace audio track
        const audioSender = peerConnections[peer_id].getSenders().find((s) =>
            ...
            if (audioStream) {
                ...
                audioStream.getTracks().forEach(async (track) => {
                    if (track.kind === "audio") {
                        peerConnections[peer_id].addTrack(track);
                        await handleRtcOffer(peer_id); // https://groups.google.com/g/discuss-webrtc/c/Ky3wf_hg1l8?pli=1
                        console.log("ADD AUDIO TRACK TO", {
                            peer_id,
                            peer_name,
                            audio: track,
                        });
...
async function refreshMyLocalStream(stream, localAudioTrackChange = false) {
    ...
    myVideo.style.objectFit = isScreenStreaming
        ? "contain"
        : "var(--video-object-fit)";
}
...
function startRecordingTimer() {
    ...
    recTimer = setInterval(function printTime() {
        if (!isStreamRecordingPaused) {
            ...
            myVideoParagraph.innerText = myPeerName + " 🔴 REC " +
                recTimeElapsed;
...
function startMobileRecording(options, audioMixerTracks) {
    try {
        ...
        console.log(
            "New Cam Media Stream tracks  --->",
            recCamStream.getTracks(),
        );
        ...
        handleRecordingError(
            "Unable to record the camera + audio: " + err,
            false,
        );
    }
}
...
function downloadRecordedStream() {
    try {
        ...
        lastRecordingInfo.innerHTML =
            `<br/>Last recording info: ${recordingInfo}`;
...
function toggleChatPin() {
    if (isCaptionPinned) {
        return userLog(
            "toast",
            "Please unpin the Caption that appears to be currently pinned",
        );
    }
...
async function sendChatMessage() {
    if (!thereArePeerConnections() && !isChatGPTOn) {
        ...
        return userLog(
            "info",
            "Can't send message, no participants in the room",
        );
    }
...
function handleDataChannelChat(dataMessage) {
    ...
    if (from_peer_name != msgFrom) {
        console.log("Fake message detected", {
            realFrom: from_peer_name,
            fakeFrom: msgFrom,
            msg: msg,
        });
        return;
...
function appendMessage(
    ...
    const isValidPrivateMessage = getPrivateMsg && getMsgId != null &&
        getMsgId != myPeerId;
...
function speechMessage(newMsg = true, from, msg) {
    ...
    speech.text = (newMsg ? "New" : "") + " message from:" + from +
        ". The message is:" + msg;
    speech.rate = 0.9;
...
function addMsgerPrivateBtn(msgerPrivateBtn, msgerPrivateMsgInput, peerId) {
    ...
    function sendPrivateMessage() {
        msgerPrivateMsgInput.value = filterXSS(
            msgerPrivateMsgInput.value.trim(),
        );
    ...
    signalingSocket
        ...
        .then(
            function (completion) {
                ...
                cleanMessageInput();
                speechInMessages
                    ? speechMessage(true, "ChatGPT", message)
                    : playSound("message");
            }.bind(this),
        )
        .catch((err) => {
...
function setPeerAudioStatus(peer_id, status) {
    ...
    if (peerAudioStatus) {
        peerAudioStatus.className = status
            ? className.audioOn
            : className.audioOff;
        setTippy(
            peerAudioStatus,
...
function handlePeerAudioBtn(peer_id) {
    ...
    peerAudioBtn.onclick = () => {
        if (peerAudioBtn.className === className.audioOn) {
            isPresenter ? disablePeer(peer_id, "audio") : msgPopup(
                "warning",
...
function handlePeerVideoBtn(peer_id) {
    ...
    peerVideoBtn.onclick = () => {
        if (peerVideoBtn.className === className.videoOn) {
            isPresenter ? disablePeer(peer_id, "video") : msgPopup(
                "warning",
...
function setPeerVideoStatus(peer_id, status) {
    ...
    if (status) {
        ...
        if (peerVideoAvatarImage) {
            elemDisplay(peerVideoAvatarImage, true, "block");
        }
        if (peerVideoStatus) {
...
function handleScreenStart(peer_id) {
    ...
    if (remoteVideoStatusBtn) {
        ...
        setTippy(
            ...
            "Participant screen share is on",
            "bottom",
        );
    }
...
function handleDataChannelFileSharing(data) {
    ...
    receiveFilePercentage.innerText = "Receive progress: " +
...
function sendFileData(peer_id, broadcast) {
    console.log(
        "Send file " + fileToSend.name + " size " +
            bytesToSize(fileToSend.size) + " type " + fileToSend.type,
    );

    sendInProgress = true;

    sendFileInfo.innerText = "File name: " +
        fileToSend.name +
        "\n" +
        "File type: " +
        fileToSend.type +
    ...
    fileReader.addEventListener(
        "error",
        (err) => console.error("fileReader error", err),
    );
    fileReader.addEventListener(
        "abort",
        (e) => console.log("fileReader aborted", e),
    );
    fileReader.addEventListener("load", (e) => {
        ...
        sendFilePercentage.innerText = "Send progress: " +
            ((offset / fileToSend.size) * 100).toFixed(2) + "%";
    ...
    if (isImageURL(incomingFileInfo.file.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                position: "center",
                title: "Received file",
                text: incomingFileInfo.file.fileName + " size " +
                    bytesToSize(incomingFileInfo.file.fileSize),
    ...
    Swal.fire({
        ...
        if (result.value) {
            ...
            */
            ...
            const is_youtube = getVideoType(result.value) == "na"
                ? true
                : false;
...
function handlePeerKickOutBtn(peer_id) {
    ...
    peerKickOutBtn.addEventListener("click", (e) => {
        isPresenter ? kickOut(peer_id) : msgPopup(
            "warning",
    ...
    switch (action) {
        case "start":
            ...
            Swal.fire({
                ...
                text:
                    `${peer_name} wants to start the captions for this session. Would you like to enable them?`,
                confirmButtonText: `Yes`,
                denyButtonText: `No`,
                showClass: { popup: "animate__animated animate__fadeInDown" },
                hideClass: { popup: "animate__animated animate__fadeOutUp" },
                ...
                if (result.isConfirmed) {
                    ...
                    if (!recognitionRunning) {
                        const {
                            ...
                            recognitionDialectIndex,
                        } = config.data;
                        recognitionLanguage.selectedIndex =
                            recognitionLanguageIndex;
                        updateCountry();
                        recognitionDialect.selectedIndex =
                            recognitionDialectIndex;
...
function handlePeerVolume(data) {
    ...
    if (!isAudioPitchBar) {
        ...
        if (
            remotePeerVideo && remotePeerVideo.classList.contains("videoCircle")
        ) {
